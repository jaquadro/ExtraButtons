...
public final class ModBlocks
{
    public static Block
        ...
        CHERRY_PANEL_BUTTON = null,
        DARK_OAK_PANEL_BUTTON = null,
        MANGROVE_PANEL_BUTTON = null,
        BAMBOO_PANEL_BUTTON = null,
        CRIMSON_PANEL_BUTTON = null,
        WARPED_PANEL_BUTTON = null,
    ...
    public static void initialize() {
        ...
        ItemGroupEvents.modifyEntriesEvent(CreativeModeTabs.REDSTONE_BLOCKS).register(group -> {
            group.accept(CAPACITIVE_TOUCH_BLOCK);
            group.accept(STONE_PANEL_BUTTON);
            group.accept(OAK_PANEL_BUTTON);
            group.accept(DELAY_BUTTON_BLOCK);
            group.accept(ENTITY_DETECTOR_RAIL);
        });

        ItemGroupEvents.modifyEntriesEvent(CreativeModeTabs.COLORED_BLOCKS).register(group -> {
            group.accept(WHITE_TOGGLE_BUTTON);
            group.accept(ORANGE_TOGGLE_BUTTON);
            group.accept(MAGENTA_TOGGLE_BUTTON);
            group.accept(LIGHT_BLUE_TOGGLE_BUTTON);
            group.accept(YELLOW_TOGGLE_BUTTON);
            group.accept(LIME_TOGGLE_BUTTON);
            group.accept(PINK_TOGGLE_BUTTON);
            group.accept(GRAY_TOGGLE_BUTTON);
            group.accept(LIGHT_GRAY_TOGGLE_BUTTON);
            group.accept(CYAN_TOGGLE_BUTTON);
            group.accept(PURPLE_TOGGLE_BUTTON);
            group.accept(BLUE_TOGGLE_BUTTON);
            group.accept(BROWN_TOGGLE_BUTTON);
            group.accept(GREEN_TOGGLE_BUTTON);
            group.accept(RED_TOGGLE_BUTTON);
            group.accept(BLACK_TOGGLE_BUTTON);
        });
    }
    ...
    private static Block register(String name, Block block, boolean registerItem) {
        ResourceLocation id = ResourceLocation.fromNamespaceAndPath(ExtraButtons.MOD_ID, name);
        if (registerItem) {
    ...
    private static Block registerToggleButtonBlock(String name, DyeColor color) {
        return register(name, new ToggleButtonBlock(color, BlockBehaviour.Properties.of()
            .noCollission().strength(0.5F).pushReaction(PushReaction.DESTROY)));
    }

    private static Block registerWoodPanelButtonBlock(String name) {
        return register(name, new PanelButtonBlock(BlockBehaviour.Properties.of()
            .noCollission().strength(0.5f).pushReaction(PushReaction.DESTROY), BlockSetType.OAK, 30));
    }

    private static Block registerStonePanelButtonBlock(String name) {
        return register(name, new PanelButtonBlock(BlockBehaviour.Properties.of()
            .noCollission().strength(0.5f).pushReaction(PushReaction.DESTROY), BlockSetType.STONE, 20));
    }

    private static Block registerCapacitiveTouchBlock(String name) {
        return register(name, new CapacitiveTouchBlock(BlockBehaviour.Properties.of()));
    }

    private static Block registerDelayButtonBlock(String name) {
        return register(name, new DelayButtonBlock(BlockBehaviour.Properties.of().pushReaction(PushReaction.DESTROY)));
    }

    private static Block registerEntityDetectorRailBlock(String name) {
        return register(name, new EntityDetectorRailBlock(BlockBehaviour.Properties.of()
            .noCollission().strength(0.7F).sound(SoundType.METAL).pushReaction(PushReaction.DESTROY)));
    }

    private static Block registerEntityPoweredRailBlock(String name) {
        return register(name, new EntityPoweredRailBlock(BlockBehaviour.Properties.of()
            .noCollission().strength(0.7F).sound(SoundType.METAL).pushReaction(PushReaction.DESTROY)));
    }
